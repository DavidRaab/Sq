...
A sequence is a generator for iterators. But you never use the iterators
directly. Instead you do operation on a sequence. A sequence don't
do anything immiadetely. It will wait with its computation until you
request values from it.

As a sequence generates iterators when needed. You always can execute a
sequence as often as you want. From its usage it looks like an immutable
iterator.
