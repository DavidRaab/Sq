...
it is automatically loaded by loading C<Sq>. Maybe at a later point it is lazy
loaded. When you use C<< Sq->fs >> then future code should still work when
...
When a file doesn't exists then you just get an empty sequence. No error, warning
...
want that a file is only read into memory a single time, and only as much
is needed. Then you either should C<cache> the sequence.
...
Reads a file as a binary file. With C<$size> you specify how much bytes are
read at once.
