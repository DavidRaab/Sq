...
overview of the functionality of Sq itself.
    ...
    dispatch($f_key,$hash) -> sub($x)              # creates a function that uses $f_key to create a key
...
=head2 Sq->math
    ...
    my $num       = Sq->math->fac     ($int)
    my $bool      = Sq->math->is_prime($number)
    my $array     = Sq->math->permute_count_up($array)
    my $str       = Sq->math->to_num_system($str_places, $num)
    my $str       = Sq->math->to_binary($num)
    my $str       = Sq->math->to_hex($num)
    my $int_array = Sq->math->divide_spread($n,$d)
    my $int_array = Sq->math->divide_symmetric($n,$d)
...
=head2 Sq->rand
    ...
    my $seq = Sq->rand->str($amount)                # sequence of random strings of length $amount
...
=head2 Sq->sys
    ...
    my $path    = Sq->sys->sys->dir()     # Path::Tiny object from directory executable is in
...
Variables that start with C<$f_> are functions.
    ...
    Array->init2d    ($width, $height, $init)
