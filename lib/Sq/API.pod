...
=head2 Sq->io
    ...
    Sq->io->csv_read($file) # returns sequence of hashes from a CSV file
...
=head2 Sq->rand
    ...
    Sq->rand->str($min, $max)             # sequence of random strings of length $min-$max
    Sq->rand->str($min, $max, $str_chars) # sequence of random strings of length $min-$max that uses characters from $str_chars
...
Variables that start with C<$f_> are functions.
    ...
    combine          ($array_of_hash, $unique_key, @fields)
    contains         ($array, $any)
    count            ($array)
