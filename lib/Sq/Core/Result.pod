...
Perl avoids this problem be having some global accessible variables. For example
we can access C<$!> that contains the error of the last called function that
set this variable. But sometimes some module also use other gloabl variables, for
