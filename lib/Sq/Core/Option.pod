...
A Option is an alternative to represent undef/null values. By default every
...
A option is a way to make it explicitly visible that a function sometimes returns
...
Same as C<or> but instead of passing a value that is returned in the case of
C<None>, a function is called and the value of it is returned. This is useful
in two cases.
...
When you have mutable values that should be different.
...
=back
    ...
    # returns current time of None
    my $x = $opt->or_with(sub { scalar localtime });

=head2 map($optional, $f)

C<map> runs a function that don't know about optionals on the value it contains
and returns the new optional value or C<None> if it was C<None>. C<map> is
good for functions with the signature C<<'a -> 'b>>
    ...
    my $x = $opt    ->map($incr);
    my $x = Some(0) ->map($incr); # Some(1)
    my $x = Some(10)->map($incr); # Some(11)
    my $x = None    ->map($incr); # None
...
optional values, you use this function. Returns an optional value again.
    ...
    my $x = Option::map2($optA, $optB, $add);
    my $x = $optA->map2($optB, $add);

    my $x = Some(10)->map2(Some(3), $add); # Some(13)
    my $x = None    ->map2(Some(3), $add); # None
...
value. It is good for functions with the signature C<<< 'a -> Option<'b> >>>
    ...
    my $x = Some("10")->bind($parse_int); # Some(10)
    my $x = Some("ab")->bind($parse_int); # None
    my $x = None      ->bind($parse_int); # None

    # compare it when you have used map instead of bind
    my $x = Some("10")->map($parse_int);  # Some(Some(10))
    my $x = Some("ab")->map($parse_int);  # Some(None)
    my $x = None      ->map($parse_int);  # None
...
flattens its down to just a single optional value. One reason why you could
    ...
    my $x = Some(Some(10))             # Some(10)
    my $x = Some(Some(Some(Some(10)))) # Some(10)
    my $x = Some(Some(Some(None)))     # None
...
returns C<$state> or whatever C<$f> returns. The types of both should be the
same, oterwise this function hardly makes any sense.

    my $add = sub($state, $x) { $state + $x };

    my $x = $opt    ->fold(100, $add);
    my $x = Some(10)->fold(100, $add); # 110
    my $x = None    ->fold(100, $add); # 100
