...
except C<undef> turns into a valid value. C<Some> has a detection that prohibits
creating nested optional values. When you pass an optional to C<Some> then this
optional is returned as-is.
...
=head2 flatten($opt) : $opt

Since the latest changes to C<Option> this method has become B<DEPRECATED>.
It still exists, but usually is not needed anymore, as you cannot create
nested options anymore. You will get the result shown here without calling
C<flatten>. Currently I am in an evaluation stage and see if that change
creates any problem. If not, then this function will be removed in the future.
...
=head2 dump($opt, $inline=60, $depth=0) : $str
...
=head2 dumpw($opt, $inline=60, $depth=0) : void
...
=head2 Option->extract_array(@args) : ($bool, @xs)

This works like C<extract> but with a lambda that is supposed to return multiple
values, or an optional value containing an array. It is used as an example
in C<< Array->unfold >>.

    # the lambda returns two values or undef
    ...
    # returns Some/None but in the Some case it needs to wrap values in an array.
...
When you want to create functions that also behave in such a way, then you
use C<< Option->extract_array >>. Here is how C<unfold> is implemented.
...
Here C<$f_opt> is supposed to either return a defined value or C<undef>. Or
it should return C<Some($array)> or C<None>.

Passing the result to C<< Option->extract_array >> does the magic for you,
it first returns a boolean flag indicating if you got Some value or not,
all extra parameters are whatever the lambda returned in list context or
the extracted values from C<Some($array)>.
