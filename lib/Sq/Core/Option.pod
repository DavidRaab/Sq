...
back if the value is an optional, and if it is B<Some> value.
    ...
    Option::is_some(Some(1));     # 1
    Option::is_some(None);        # 0
    Option::is_some(Some(undef)); # 0
    Option::is_some("");          # 0
    Option::is_some(0);           # 0
    Option::is_some([]);          # 0
    Option::is_some({});          # 0
...
Calling this method in a functional-style has the benefit that it also does
a proper type-checking of an optional, you can pass any value to it and get
back if the value is an optional, and if it is B<None> value.

    Option::is_none(Some({}));    # 0
    Option::is_none(Some(1));     # 0
    Option::is_none(None);        # 1
    Option::is_none(Some(undef)); # 1
    Option::is_none("");          # 0
    Option::is_none(0);           # 0
    Option::is_none([]);          # 0
    Option::is_none({});          # 0
...
an empty string for example is not an optional to begin with. Currently I will
consider if those cases also should return C<1>.
...
to the idea of first checking with C<< Option->is_opt($any) >> if it is an
Optional value, then they maybe call C<< $any->is_some >> and finally do a
C<< $any->get >> to get the value. B<Don't do that>.
