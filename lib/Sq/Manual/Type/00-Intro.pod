...
* static vs dynamic
* functions vs type declaration
* runtime vs. compile-time (AOP)
...
But consider that usually data is more often accessed/written or read from and
including type-checks at every place can be a real burdon that also makes building
the data in itself very slow.