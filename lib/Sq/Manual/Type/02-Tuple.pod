...
hand we just get three elements. The type ov the above C<data> would be
C<< list<string * int> >>.
...
are arrays of a specifix fixed-size array and it contains exact values.

And in C<Sq::Type> we can use C<t_tuple> to exactly create such a type.
We just can write.
...
When you ever programed in Lisp. Well, it's exactly the same in Lisp/Scheme!

In Perl you could achieve the same by writing.
...
Maybe you wonder. How can you call Classes, Methods, functions that are defined
in C# from F#? Well here it is.

    Class.StaticFunc (x,y,z)

but. You see what happens here? Actualy from F# perspective this is not
a function with three arguments. It's a function call only having a single
argument. but instead you give it a tuple containg three elements.

This is actually the same as you see in mathematics. For example you see
stuff like

    f (x,y) = x * y

well in F# you just write

    f (x,y) = x * y

and it would be the same. A function C<f> expecting a single argument. A Tuple
...
as a Tuple and every function has a single output. So far you have seen C<sig>
...
a function C<sigt> that doesn't do that. But then you ony can specify a single
...
maybe you wonder why you ever want to use C<sigt>? Well, technically
because the tuple is not build for you, you actually have more flexibility
when it comes to allowing different function signatures for a function.
...
But the function C<to_array> still can do something. Usually you have code like
...
so only the first 10 elements of a sequence a put into an array. This behaviour
...
Here we use C<t_or> to describe an alternative. So the function must be
either just accept an C<Array> or it must accept an C<Array,int> as an
argument.

This idea that there are only functions with a single argument and all input
arguments are just passed as an array is what makes checking argument so easy.
So all your input is just an array and you can consider a function argument
