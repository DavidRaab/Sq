...
how about a health type?
    ...
    my $has_health = t_hash(
        t_key(health => t_hash(t_keys(
            min     => t_num,
            max     => t_num,
            current => t_num.
        ))),
...
It also makes tetsing very easy. You just need to compare data! That's all.
...
Executing this code now prints the following.
    ...
    Type Error: tuple: Index 1: int: Not an integer at ./sq_test.pl line 7.
...
The whole idea of B<loading> is actually what it seperates it from all other
modules out there. The idea is that when you write code. Usually you are in
development, testing and so on. Then you load a I<Signature> file that does
a correct type-checking. This type-checking helps you in writing correct code.

But once your code works and you go into I<production>, I<release> or whatever you
...
This is all you need to write. It loads a C<sig> function that allows you to
add type-checking to any function defined in Perl. When you wonder how it works.

It's the same technique used by the  L<Memoize> module. That means a function is replaced
in the symbol table and is replaced with a new function that has the type-check
added. This kind of programming is also sometimes named I<Aspect-Oriented-Programming>.

=head2 Array::sum

Let's fo through another function to fully understand the whole benefits of
...
dynamic-typed as you expect. because passing anything else to C<sum> then
...
Here are some other kinds of code examples and the error-messages.
    ...
    # Type Error: tuple: Index 0: even_sized: Array not even-sized at ./sq_test.pl line 11.

    my $sorted = sq([1,2,3,"foo" ])->sort_num;
    # Type Error: tuple: Index 0: all: index 3: num: Not a number 'foo' at ./sq_test.pl line 11.

    my $matches = sq(["12-12", "10-10", []])->regex_match(qr/\A(\d\d)-(\d\d)\z/);
    # Type Error: tuple: Index 0: all: index 2: str: Not a string at ./sq_test.pl line 11.
...
This is also the case for the other stuff. When you try to sort numbers and you
...
When you encounter a number you usually want to abort.

Correct code would be that you first C<filter/grep> for a number, do some
...
throws an error or other kind of I<garbage programming>. When you want to return
Errors you use C<Result> or C<Option> not throw exceptions!

Write code in such a way that you always expect correct things are passed and
just something as an array, hash and just use the keys like you need them.

This way you get the benefits if both worlds! You write like in a dynamic-typed language.
...
in your code, and get fast code.
