...
replicate, rev, skip, take, slice, split, sum, sum_by, windowed
...
We have: new, init, unfold, range, rage_step, replicate, concat
...
And I don't think that fits Perl good. Look I absolutely like type-checking.
This module is actually created and brings idea from B<F#> or in general
B<ML> languages to Perl. But still it doesn't try to add type-checking.
...
For example as long as you develop you can add the type-checking and ensure your
code is correct and run your test-suite with it. But in production code when
speed matters those type-checks are removed.

=head2 Another Example

Let's look at another example on why I think this programming model us superior.
...
Now let's assume i want to get the title with the maximum running time. I
...
But again. Why the hell do you wanna do that? USE YOUR FUNCTION LANGUAGE
FEATURES!!!
...
speed writing a C<max> function to pick the longest track for the thousand
time.
...
are mutable you use them like you do immutable programming.
...
or it turns B<None> when the key C<tracks> in the Hash does not exists
or is not defined.
...
that B<one additional thing> while you get/set it.

Oh gosh, and you know what. Whenever that happens and a getter/setter does
more than just getting or setting a variable, it ALWAYS CAUSES PROBLEMS!

You just think you retrieve that one thing, or set one kind of field, but
not it does not.
...
it just calculates a whole value again and again whenever you call it and you
don't have a fucking clue that it does that!

I mean look at C<Moose>. When you create classes with C<Moose> what do you
do 99% percent of the time with those getters/setters? Right you do absolute
nothing. You just use it as a convenience way so you don't need to write
the C<new> method and just use every field to just set/get the value,
you maybe like it that it does some basic type validation. And that's it.

But again, why? Most of the time classes are just wrappers around hashes.
And those getters/setters are just functions to get and set a value in a
hash, that's it. But it comes at a high price you pay just you can
maybe type two characters less. Remember

    my $time = $obj ->time;
    my $time = $hash->{time};

One reason why we used classes in Perl is because its dynamic typing nature
of a Hash on how Perl works it is prone to typing errors. Let's assume we
...
because what we have hear is basically a programming error. We try to
access a field that doesn't exists.

It's like not using C<use strict> and suddenly every variable you use
that was not defined is not even an error anymore. Horrible. But Python
developers will tell you it's not a problem.
