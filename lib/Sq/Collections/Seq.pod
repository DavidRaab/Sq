...
sequence, but when C<$choose> returns C<undef> then the item is skipped in the
final sequence. This function is a combination of C<map> followed by C<filter>
in a single call.
...
Concatenates all elements of a sequence to a string with C<$separator>.
    ...
    # "foo-bar"
    my $str = Seq->new("foo", "bar)->join(-);

=head2 first($seq, $default) : $x
...
=head2 fold($seq, $state, $folder) : $state
...
=head2 fold_mut($seq, $state, $folder) : $state
