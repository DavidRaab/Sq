...
for an array.
    ...
    # seq { 2,4,6,8,10 }
    my $seq = Seq->range(1,10)->filter(sub($x) { $x % 2 == 0 });
...
This function is a combination of C<map> followed by C<filter> in a single call.
...
=head2 regex_match($seq_of_strings, $regex) : $seq_of_array
...
=head2 dump($seq, $inline=60, $depth=0) : $str
...
=head2 dumpw($seq, $inline=60, $depth=0) : void
