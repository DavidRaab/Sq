...
Turns array into a hash. Basically the same as you are used in Perl as assigning
an array to a hash. But the returning Hash is blessed with the 'Hash' package.

    my $hash = $array->as_hash;

similar to, but without Hash blessing.
