...
C<choose> combines C<map> and C<filter> in a single operation. Every value
...
=head2 filter_e($array, $expr) : $array

Same as C<filter> but expects a string as C<$expr>. It uses string-eval to build
the final filtering instead of calling a lamda function. This overall results
