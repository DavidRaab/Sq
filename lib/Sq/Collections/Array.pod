...
No re-implementation of an Array, as it is not needed. Would be probably to slow
...
=head2 Array->from_array($aref) -> $array
...
=head2 Array->init($count, $f) -> $array

Generates a new C<Array> with C<$count> entries. An entry generating function
C<$f> is called by passing it the current index. The function can return multiple
...
=head2 Array->unfold($state, $f) -> $array
...
=head2 cartesian($arrayA, $arrayB) -> $array
...
=head2 length($array) : Array<'a> -> int
