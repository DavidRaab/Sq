...
But when C<$count> is being passed than a new Array must be created. When C<$count>
is smaller or equal to C<0> than a new empty array is returned. Otherwise C<$count>
elements are copied to a new array.

It only copies as much entries as the original array has.
    ...
    my $array = $data->to_array(1000); # [1..100]
