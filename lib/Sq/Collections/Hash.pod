...
An Sq Hash always should and can be used wherever a Hash reference is
expected. This works as long a module does not any kind of pseudo type-checking
and just uses it as a hash-reference.

=item Performance

No re-implementation of a Hash, as it is not needed. Would be probably to slow
...
union of two hashes, when a key exists in both hashes then C<$f_value> is called
with C<$key,$a_value,$b_value>. C<$f_value> then returns the B<value> that should
be used in the new generated hash.
...
Converters are methods that either transforms or extract data from a Hash. Usually
returning something different than a Hash. Like METHODS they also
...
returns all the keys of a Hash. Same as perl built-in C<keys> functions.

    for my $key ( @{ $hash->keys } ) {
        say $key;
    }

    # keys returns an Sq Array
...
=head2 find($hash, $predicate) : Option<[$key,$value]>

Goes through every key and value of a Hash, passes it to C<$predicate> and
returns the first key,value that C<$predicate> returns a truish value. When
C<$predicate> does not find any key,value than it returns C<None>.
...
=head2 pick($hash, $f_opt) : Option<$x>

Similar to find as it searches for a key,value pair. But instead of providing
