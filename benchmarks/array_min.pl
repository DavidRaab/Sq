...
sub min1($array) {
    my $len = @$array;
    return None()            if $len == 0;
    return Some($array->[0]) if $len == 1;
...
sub min4($array) {
    ...
    return defined $min ? Some($min) : None;
}

# Testing
my @funcs = qw(min1 min2 min3 min4);
